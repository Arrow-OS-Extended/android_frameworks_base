...
public class PropsHooksUtils {
    ...
    public static final String ENABLE_PER_APP_PROP_OPTIONS = "persist.sys.perapp.enabled";
    ...
    public static void setPerAppProps(String packageName) {
        if (!SystemProperties.getBoolean(ENABLE_PER_APP_PROP_OPTIONS, false)) {
            return;
        }
        if (packageName == null || packageName.isEmpty()) {
            return;
        }
        Map<String, String> perAppPropsToChange = new HashMap<>();
        ...
        for (String key : keys) {
            ...
            if (value != null && !value.isEmpty()) {
                perAppPropsToChange.put(key, value);
                if (DEBUG) Log.d(TAG, "Got system property: " + systemPropertyKey + " = " + value);
            }
        }
        if (!perAppPropsToChange.isEmpty()) {
            if (DEBUG) Log.d(TAG, "Defining props for: " + packageName);
            for (Map.Entry<String, String> prop : perAppPropsToChange.entrySet()) {
                String key = prop.getKey();
